...
func (redis *RedisApp) Info() *RedisImp {
	info := &RedisImp{
		BaseTune: TuneInfo{
			TuneName:      "redis",
			WorkDirectory: "mkdir -p /tmp/tune/ && cd /tmp/tune/ && [ -e redis.tar.gz ] && tar -xzvf redis.tar.gz || ( curl -OJ http://" + config.Config().HttpServer.Addr + "/api/v1/download/redis.tar.gz && tar -xzvf redis.tar.gz)",
