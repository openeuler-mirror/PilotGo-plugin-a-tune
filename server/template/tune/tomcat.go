...
func (tomcat *TomcatApp) Info() *TomcatImp {
	info := &TomcatImp{
		BaseTune: TuneInfo{
			TuneName:      "tomcat",
			WorkDirectory: "mkdir -p /tmp/tune/ && cd /tmp/tune/ && [ -e tomcat.tar.gz ] && tar -xzvf tomcat.tar.gz || ( curl -OJ http://" + config.Config().HttpServer.Addr + "/api/v1/download/tomcat.tar.gz && tar -xzvf tomcat.tar.gz)",
