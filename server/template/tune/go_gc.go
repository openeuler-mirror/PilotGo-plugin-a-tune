...
func (g *GoGcApp) Info() *TuneInfo {
	info := &TuneInfo{
		TuneName:      "go_gc",
		WorkDirectory: "mkdir -p /tmp/tune/ && cd /tmp/tune/ && [ -e go_gc.tar.gz ] && tar -xzvf go_gc.tar.gz || ( curl -OJ http://" + config.Config().HttpServer.Addr + "/api/v1/download/go_gc.tar.gz && tar -xzvf go_gc.tar.gz)",
