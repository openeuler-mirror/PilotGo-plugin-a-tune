...
func (kps *KeyParametersSelectApp) Info() *TuneInfo {
	info := &TuneInfo{
		...
		WorkDirectory: "mkdir -p /tmp/tune/ && cd /tmp/tune/ && [ -e key_parameters_select.tar.gz ] && tar -xzvf key_parameters_select.tar.gz || ( curl -OJ http://" + config.Config().HttpServer.Addr + "/api/v1/download/key_parameters_select.tar.gz && tar -xzvf key_parameters_select.tar.gz)",
