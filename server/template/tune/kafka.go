...
func (k *KafkaApp) Info() *KafkaImp {
	info := &KafkaImp{
		BaseTune: TuneInfo{
			TuneName:      "kafka",
			WorkDirectory: "mkdir -p /tmp/tune/ && cd /tmp/tune/ && [ -e kafka.tar.gz ] && tar -xzvf kafka.tar.gz || ( curl -OJ http://" + config.Config().HttpServer.Addr + "/api/v1/download/kafka.tar.gz && tar -xzvf kafka.tar.gz)",
